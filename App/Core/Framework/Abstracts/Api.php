...
abstract class Api extends Channel
{
	...
	{
		return Router::getInstance()->getParameters()['POST'];
	}

	public function retrieveGET()
	{
		return Router::getInstance()->getParameters()['GET'];
	}

	public function APIErrorHandler($errorNumber, $errorMessage, $errorFile, $errorLine)
	{
		if (Configuration::AUTOLOG_ERRORS) {
			$errorData = "\nError: $errorNumber\nMessage: $errorMessage\nOn: $errorFile($errorLine)\nProcessID: " . getmypid();
			Logger::LogError("APIErrorHandler", $errorData);
		}
		try {
			Actions::clearOutputBuffer();
			http_response_code(500);
			echo json_encode(new APIResponse(SharedConsts::HTTP_RESPONSE_INTERNAL_SERVER_ERROR, Actions::printLocalized(Strings::API_UNHANDLED_EXCEPTION)));
		} catch (\Throwable $th) {
			Logger::LogError(null, $th . "\n" . Logger::generateCallTraceString());
		} finally {
			die();
		}
	}
